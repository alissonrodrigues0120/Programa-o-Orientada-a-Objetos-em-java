...
class UFC {
    ...
    public Funcionario getFuncionario(String nome) {
        for (Funcionario alguem : funcionarios.values()){
		    if(alguem.getNome().equals(nome)){
			    return alguem;
		    }
	    }
    ...
    public Aluno getAluno(String nome) {
	    for (Aluno alguem : alunos.values()){
		    if(alguem.getNome().equals(nome)){
			    return alguem;
		    }
	    }
...
class Sistema {
    ...
    public void addUsuario(String nome, String senha) {
	    if(ufc.getFuncionario(nome) != null){
	              Funcionario funcionarioEncontrado = ufc.getFuncionario(nome);
	   
                if( ufc.getFuncionario(nome) instanceof Terceirizado){
	               throw new MsgException("fail: terc nao pode ser cadastrado no sistema");
               }else{
                   
                   if(funcionarioEncontrado instanceof Professor){
                       Professor prof = (Professor) funcionarioEncontrado;
                       usuarios.put(nome, prof );
                   }else{
                        STA service = (STA) funcionarioEncontrado; 
                       usuarios.put(nome, service);
                   }
                   
                   
                   usuarios.get(nome).setSenha(senha);
               }
               
	       }else if(ufc.getAluno(nome) != null){
	        usuarios.put(nome, ufc.getAluno(nome));
	        usuarios.get(nome).setSenha(senha);
    ...
    public String showUser(String nome){
        if(getUsuario(nome) != null){
            String ss = "";
            ss += usuarios.get(nome) + ":" + usuarios.get(nome).getSenha() + ":" + usuarios.get(nome).getLogado();
            return ss;
        }else {
